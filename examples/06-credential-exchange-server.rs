...
use ockam::identity::{
    AuthorityService, CredentialsIssuerClient, SecureChannelListenerOptions, SecureChannelOptions,
    TrustContext,
};
...
/// examples/06-credential-exchange-server.rs
/// This node starts a tcp listener on 4000, a secure channel listener, and an echoer worker.
/// It also verifies that it can authenticate its credentials against the issuer.
...
async fn main(ctx: Context) -> Result<()> {
    ...
    let output_msg = format!("❓ Credential to verify:\n{credential}");
